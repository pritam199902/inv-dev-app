...
const PurchaseEntry = (props) => {
  ...
  const [purBillData, setPurBillData] = useState({
    ...
    // {
    // // id: 1,
    // billId: Math.floor(Math.random() * 10000000000 + 1),
    // billDate: "",
    // vendorId: "",
    // itemData: [
    //   {
  ...
  const handleAddItem = () => {
    ...
    // purBillData.data.itemData.map((i, ind) => {
    // if (i.itemId !== "") {
    setCount(count + 1);
  ...
  const handleSelectItemCode = (e, i) => {
    ...
    console.log(props.data.itemList[e.target.value]);

    const { id, itemName, itemCode, itemTax, itemRate } = props.data.itemList[
      e.target.value
    ];
  ...
  const handleSelectItem = (e, i) => {
    ...
    setPurBillData({ data: purBillData.data });
    console.log(purBillData.data.itemData[i]);
  ...
  const handleAmount = (i) => {
    ...
    //   return (
    //     (sub += v.total),
    //     (taxTotal += v.total * (v.itemTax / 100)),
    //     (total += v.total + taxTotal)
    //   );
    // });

    // purBillData.data.subTotal = sub;
    // purBillData.data.totalTaxAmt = Number(taxTotal.toFixed(2));
    // purBillData.data.totalAmt = Number(total.toFixed(2));
  ...
  const handlePkgAmt = (e) => {
    purBillData.data.pkgAmt = Number(e.target.value).toFixed(2);
    handleAmount(-1);
  ...
  const handleDlvAmt = (e) => {
    purBillData.data.dlvAmt = Number(e.target.value).toFixed(2);
    handleAmount(-1);
  ...
  const handleSubmit = () => {
    ...
    props.func.handleAddPurchaseBill(purBillData.data);
  };
  ...
  useEffect(() => {
    ...
    fetch("/purchase-enrty-code-generate")
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
        const lastPurBillId = data.id;
        // console.log(lastItemId);

        const peCode = "PUR00" + (Number(lastPurBillId) + 1);

        purBillData.data.billId = peCode;
        setPurBillData({ data: purBillData.data });
      });
  }, []);
  ...
  return (
    <>
      ...
      >
        <h3 style={titleText}>New Purchase Entry</h3>
      </div>
